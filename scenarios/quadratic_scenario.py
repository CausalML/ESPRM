...
from policy_learning.policy_networks import QuadraticPolicyNetwork
from util.input_embedding import to_quadratic


class GenericSimpleQuandraticScenario(object):
    ...
    def _mean_y(self, x):
        y_mean_0 = torch.matmul(to_quadratic(x), self.y_theta_0) + self.y_b_0
        y_mean_1 = torch.matmul(to_quadratic(x), self.y_theta_1) + self.y_b_1
    ...
    def _propensity(self, x):
        a_probs = torch.sigmoid(torch.matmul(to_quadratic(x), self.a_theta)
                                + self.a_b)
    ...
    def get_optimal_policy(self, policy_network_class, x_val, y_cf_val):
        policy = QuadraticPolicyNetwork(self.x_dim).double()
        if torch.cuda.is_available():
            policy = policy.cuda()
...
class SimpleQuadraticScenario(GenericSimpleQuandraticScenario):
    def __init__(self):
        x_mean = np.zeros(2)
        x_std = np.ones(2)
        x_dim = 2
        y_theta_0 = np.array([1.0, -1.0, 0.5, 0, 1.5])
        y_theta_1 = np.array([-3.0, 0.5, 1.5, -2.5, 0.5])
        y_std = np.ones(1) * 1
        a_theta = np.array([1.5, 2.0, -2.0, -0.5, 1.5])

        GenericSimpleQuandraticScenario.__init__(
            self, x_mean=x_mean, x_std=x_std, x_dim=x_dim, y_theta_0=y_theta_0,
            y_theta_1=y_theta_1, y_std=y_std, a_theta=a_theta)


class RandomQuadraticScenario(GenericSimpleQuandraticScenario):
    def __init__(self):
        setup_args = RandomQuadraticScenario._get_random_setup_args()
        GenericSimpleQuandraticScenario.__init__(self, **setup_args)
    ...
    def _get_random_setup_args():
        setup_args = {
            ...
            "x_std": np.ones(2),
            "x_dim": 2,
            "y_theta_0": np.random.randn(5),
            "y_theta_1": np.random.randn(5),
            "y_std": np.ones(1) * 1,
            "a_theta": np.random.randn(5),
